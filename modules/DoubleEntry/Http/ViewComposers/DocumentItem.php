...
class DocumentItem
{
    ...
    public function compose(View $document_item_view)
    {
        ...
        if ($request->segment(2) == 'purchases' || $request->segment(2) == 'expenses' || $request->segment(2) == 'credit-debit-notes') {
        ...
        if ($request->segment(2) == 'purchases' || $request->segment(2) == 'expenses') {
        ...
        if ($request->routeIs('invoices.edit')
            ...
            || $request->routeIs('credit-debit-notes.credit-notes.edit')
            || $request->routeIs('credit-debit-notes.debit-notes.edit')
            || $request->routeIs('expenses.expense-claims.edit')
            ) {
            $document = $request->route(Str::singular((string) Str::slug($request->segment(3), '_')));
        ...
        if ($request->routeIs('recurring-invoices.edit') || $request->routeIs('recurring-bills.edit')) {
            $document = $request->route(Str::singular((string) Str::slug($request->segment(3), '_')));

            foreach ($document->items as $item) {
                $account_id = RecurringLedger::record($item->id, 'App\Models\Document\DocumentItem')->value('account_id');
        ...
        if ($request->routeIs('bills.*')
            || $request->routeIs('recurring-bills.*')
            || $request->routeIs('credit-debit-notes.debit-notes.*') 
