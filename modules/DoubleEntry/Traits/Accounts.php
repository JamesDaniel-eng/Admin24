...
trait Accounts
{
    ...
    public function findImportedTypeId($row)
    {
        $type = Type::all()->first(function ($type) use ($row) {
            return trans($type->name) == $row['type'];
        });
        ...
        $class_id = $this->findImportedClassId($row);

        if (!is_null($class_id)) {
            return Type::where('class_id', $class_id)->first()?->id ?? null;
        }

        return null;
    }

    public function findImportedClassId($row)
    {
        $class = DEClass::all()->first(function ($class) use ($row) {
            return trans($class->name) == $row['class'];
        });
