...
class BalanceSheet extends Report
{
    ...
    protected function calculateCurrentYearEarnings($start_date, $end_date, $basis)
    {
        ...
        $earning = new Account();
        $earning->company_id = $this->model->company_id;
        $earning->name = trans('double-entry::general.current_year_earnings');

        // This change was made to the ledger status in the credit debit notes module. The calculation error in negative incomes was corrected.
        // $earning->de_balance = abs($income) - $expense;
        $earning->de_balance = - $expense - $income;
    ...
    public function getAccounts($type)
    {
        ...
        $accounts = $type->accounts->sortBy('code');
        ...
        if ($where_not) {
            return $accounts->where('id', '!=', $account_id);
        }
