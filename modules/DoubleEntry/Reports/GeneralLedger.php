    ...
    public function setData()
    {
        ...
        foreach ($accounts as $account) {
            ...
            foreach ($account->ledgers as $ledger) {
                ...
                // Log for debugging
                \Log::info('Ledger Entry', [
                    'transaction_label' => $ledger->transaction_label,
                    'ledgerable_type' => $ledger->ledgerable_type,
                    'ledgerable_id' => $ledger->ledgerable_id,
                    'ledgerable' => $ledger->ledgerable,
                    'ledgerable_link' => $ledger->ledgerable_link,
                ]);

                $this->row_values[$account_key][] = [
                    'issued_at' => $ledger->issued_at,
                    'transaction' => $ledger->transaction_label,
