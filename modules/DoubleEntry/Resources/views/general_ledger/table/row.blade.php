<x-table.tr>
    ...
    @if(!is_null($row['link']))
    <x-table.td class="w-2/12 col-w-2 py-2 text-left text-alignment-center text-black-400" override="class">
        <x-link href="{{ $row['link'] }}" class="text-sm sm:mt-0 sm:mb-0 leading-4" override="class">
            <x-link.hover color="to-black-400 cursor-pointer">
                {{ $row['transaction'] }}
            </x-link.hover>
        </x-link>
    </x-table.td>
    ...
    @endif
    <x-table.td class="w-4/12 col-w-4 py-2 text-left text-alignment-left text-black-400 text-xs" override="class">
        {{ $row['description'] ?? '' }}
    </x-table.td>
