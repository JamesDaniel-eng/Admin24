<x-table.tr class=" " override="class">
    <x-table.td class="w-8/12 col-w-8 ltr:text-left rtl:text-right text-alignment-left text-black-400" override="class" data-account-code="{{ $ledger->account->code }}">
        @if (! isset($print))
            <x-link
                href="{{ route('reports.show', [$ledger->id]) . '?search=de_account_id:' . $ledger->account->id }}"
                class="text-sm sm:mt-0 sm:mb-0 leading-4"
                override="class">
                <x-link.hover color="to-black-400">
                    {{ $ledger->account->code }} - {{ $ledger->account->trans_name }}
                </x-link.hover>
            </x-link>
        @elseif ($ledger->account->code)
            {{ $ledger->account->code }} - {{ $ledger->account->trans_name }}
        @else
