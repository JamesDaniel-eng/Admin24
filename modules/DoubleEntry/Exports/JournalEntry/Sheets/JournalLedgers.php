...
class JournalLedgers extends Export implements WithColumnFormatting
{
    ...
    {
        return Model::with('ledgerable', 'account', 'transaction')
            ->where('ledgerable_type', 'Modules\\DoubleEntry\\Models\\Journal')
            ->whereIn('ledgerable_id', $this->ids)
            ->cursor();
    }
    ...
    public function map($model): array
    {
        ...
        $model->number = $journal->journal_number ?? null;
        $model->transaction_number = $model->transaction->number ?? null;
        $model->account = $model->account->trans_name ?? null;
