...
class UpdateTransaction
{
    ...
    { 
        ...
        $total_ledger = RecurringLedger::record($transaction->id, get_class($transaction))
        ...
        if (empty($total_ledger)) {
            ...
            $this->dispatch(new UpdateRecurringLedger($total_ledger, $total_request));
        }

        $item_request = $this->getTransactionItemBaseRequest($request, $transaction);

        $item_ledger = RecurringLedger::record($transaction->id, get_class($transaction))
            ->where('entry_type', 'item')
            ->first();

        if (empty($item_ledger)) {
            $this->dispatch(new CreateRecurringLedger($item_request));
        } else {
            $this->dispatch(new UpdateRecurringLedger($item_ledger, $item_request));
        }
