...
class DocumentCloned
{
    ...
    public function handle($clone, $original)
    {
        ...
        for ($i = 0; $i < count($original_items); $i++) {
            if ($original->type == Document::INVOICE_RECURRING_TYPE || $original->type == Document::BILL_RECURRING_TYPE) {
                $account_id = RecurringLedger::record($original_items[$i]->id, 'App\Models\Document\DocumentItem')->value('account_id');

                if (is_null($account_id) && $original->type == Document::INVOICE_RECURRING_TYPE) {
                    $account_id = Coa::code(setting('double-entry.accounts_sales', 400))->pluck('id')->first();
                }

                if (is_null($account_id) && $original->type == Document::BILL_RECURRING_TYPE) {
                    $account_id = Coa::code(setting('double-entry.accounts_expenses', 628))->pluck('id')->first();
                }

                RecurringLedger::record($clone_items[$i]->id, 'App\Models\Document\DocumentItem')->update([
