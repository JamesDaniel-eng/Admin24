...
class RecurringLedger extends Model
{
    use Modules;

    protected $table = 'double_entry_rcr_ledger';
    ...
    public function getDescriptionAttribute()
    {
        ...
        switch ($this->ledgerable_type) {
            case 'App\Models\Banking\Transaction':
                ...
                if ($ledgerable->type == 'income-recurring') {
            ...
            case 'App\Models\Document\Document':
                ...
                if ($ledgerable->type == 'invoice-recurring') {
    ...
    public function getLedgerableLinkAttribute()
    {
        ...
        if ($this->ledgerable_type == 'App\Models\Banking\Transaction') {
            return route('recurring-transactions.show', $ledgerable->id);
        }

        if (in_array($this->ledgerable_type, $document_types) &&
            $ledgerable->type == Document::INVOICE_RECURRING_TYPE) {
            if ($this->ledgerable_type == 'App\Models\Document\Document') {
                return route('recurring-invoices.show', $ledgerable->id);
            }

            return route('recurring-invoices.show', $ledgerable->document_id);
        }

        if (in_array($this->ledgerable_type, $document_types) &&
            $ledgerable->type == Document::BILL_RECURRING_TYPE) {
            if ($this->ledgerable_type == 'App\Models\Document\Document') {
                return route('recurring-bills.show', $ledgerable->id);
            }

            return route('recurring-bills.show', $ledgerable->document_id);
        }
