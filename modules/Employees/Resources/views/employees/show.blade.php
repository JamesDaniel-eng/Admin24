<x-layouts.admin>
    ...
    <x-slot name="status">
        <div class="flex items-center">
            ...
            @if ($employee->dismissal)
                <div data-index-icon>
                    <x-tooltip placement="right" message="{{ trans('employees::employees.messages.index_dismissed') }}">
                        <span class="material-icons-outlined text-red text-sm ml-2">
                            person_remove
                        </span>
                    </x-tooltip>
                </div>
    ...
    <x-slot name="buttons">
        ...
        <x-dropdown id="item-show">
            ...
            @can('read-employees-dismissals')
                <x-dropdown.divider />

                @if ($employee->dismissal)
                    @can('delete-employees-dismissals')
                        <x-dropdown.link href="{{ route('employees.modals.dismissals.revert', $employee->dismissal->id) }}" >
                            {{ trans('employees::general.revert_dismissal') }}
                        </x-dropdown.link>
                    @endcan
                @else
                    @can('create-employees-dismissals')
                        <x-dropdown.button @click="onModalAddNew('{{ route('employees.modals.dismissals.create', ['employee_id' => $employee->id]) }}')">
                            {{ trans_choice('employees::general.dismissals', 1) }}
                        </x-dropdown.button>
                    @endcan
                @endif
            @endcan
            ...
            <x-show.content>
                @stack('employee_content_start')
                    <x-show.content.left>
                        ...
                        @stack('employee_dismissal_dismissal_type_start')
                            @can('read-employees-dismissals')
                                @if ($employee->dismissal)
                                    <div class="flex flex-col text-sm mb-5">
                                        <div class="font-medium">{{ trans('employees::general.dismissal_type') }}</div>
                                        <span>{{ $employee->dismissal->type }}</span>
                                    </div>
                                @endif
                            @endcan
                        @stack('employee_dismissal_dismissal_type_end')

                        @stack('employee_dismissal_dismissal_date_start')
                            @can('read-employees-dismissals')
                                @if ($employee->dismissal)
                                    <div class="flex flex-col text-sm mb-5">
                                        <div class="font-medium">{{ trans('employees::general.dismissal_date') }}</div>
                                        <span>
                                            <x-date date="{{ $employee->dismissal->dismissal_date }}" />
                                        </span>
                                    </div>
                                @endif
                            @endcan
                        @stack('employee_dismissal_dismissal_date_end')

                        @stack('employee_dismissal_reason_start')
                            @can('read-employees-dismissals')
                                @if (isset($employee->dismissal->reason))
                                    <div class="flex flex-col text-sm mb-5">
                                        <div class="font-medium">{{ trans_choice('employees::general.dismissals', 1) . ' ' . trans('employees::general.reason') }}</div>
                                        <span>{{ $employee->dismissal->reason }}</span>
                                    </div>
                                @endif
                            @endcan
                        @stack('employee_dismissal_reason_end')
                    </x-show.content.left>
