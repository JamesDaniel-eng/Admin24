...
class CompositeItems extends Controller
{
    ...
    /**
     * Update composite item components via AJAX
     *
     * @param IlluminateRequest $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function updateComponents(IlluminateRequest $request)
    {
        try {
            ...
            // Find the item and its composite item
            $item = Item::find($item_id);
            if (!$item || !$item->composite_item) {
                return response()->json([
                    'success' => false,
            ...
            $composite_item = $item->composite_item;

            // Delete existing components
            $composite_item->composite_items()->delete();

            // Add new components
            foreach ($components as $component) {
                if (isset($component['id']) && isset($component['quantity'])) {
                    $composite_item->composite_items()->create([
                        'item_id' => $component['id'],
                        'quantity' => $component['quantity'],
                        'warehouse_id' => $component['warehouse_id'] ?? null,
                    ]);
                }
            }
            ...
            return response()->json([
                'success' => true,
                'message' => trans('composite-items::general.components_updated_successfully')
            ]);
