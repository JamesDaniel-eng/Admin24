        ...
        <x-form.container>
            <x-form id="composite-item" route="composite-items.composite-items.store">
                ...
                @if ($inventory == true)
                    <x-form.section>
                        ...
                        <x-slot name="body">
                            ...
                            <x-form.group.checkbox name="track_inventory" id="item-track-inventory" :options="['1' => trans('inventory::items.track_inventory')]" @input="onCanTrack($event)" form-group-class="sm:col-span-2" checkbox-class="sm:col-span-6" :checked="setting('inventory.track_inventory')" />
                        </x-slot>             
                    </x-form.section>
                @endif
                ...
                <x-form.section v-if="form.track_inventory == true">
                    <x-slot name="body">
                        <x-form.group.text name="sku" label="{{ trans('inventory::general.sku')}}" form-group-class="sm:col-span-3" not-required />
                        ...
                        <x-form.group.text 
                            name="barcode" 
                            label="{{ trans('inventory::general.barcode')}}" 
                            v-show="form.track_inventory == true" 
                            form-group-class="sm:col-span-3" 
                            not-required
                            @input="onChangeBarcode()"
                        />
                ...
                <x-form.section>
                    <x-slot name="head">
                        <x-form.section.head title="{{ trans('composite-items::general.name') }}" description="{{ trans('composite-items::general.form_description.composite_items') }}" />
                    </x-slot>
