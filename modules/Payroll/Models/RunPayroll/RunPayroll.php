<?php

namespace Modules\Payroll\Models\RunPayroll;

use App\Abstracts\Model;
use App\Models\Banking\Transaction;
use App\Models\Common\Media as MediaModel;
use App\Traits\Currencies;
use App\Traits\DateTime;
use App\Traits\Media;
use Bkwld\Cloner\Cloneable;
use Illuminate\Database\Eloquent\Factories\Factory;
use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Relations\BelongsTo;
use Modules\Payroll\Traits\RunPayrolls;

class RunPayroll extends Model
{
    use HasFactory, Cloneable, Currencies, DateTime, Media, RunPayrolls;

    public const STATUS_NOT_APPROVED = 'not_approved';
    public const STATUS_APPROVED = 'approved';

    protected $table = 'payroll_run_payrolls';

    /**
     * Attributes that should be mass-assignable.
     *
     * @var array
     */
    protected $fillable = [
        'company_id',
        'name',
        'from_date',
        'to_date',
        'payment_date',
        'payment_id',
        'pay_calendar_id',
        'category_id',
        'account_id',
        'payment_method',
        'currency_code',
        'currency_rate',
        'amount',
        'status'
    ];

    protected $casts = [
        'from_date'     => 'datetime',
        'to_date'       => 'datetime',
        'payment_date'  => 'datetime',
        'amount'        => 'double',
        'currency_rate' => 'double',
        'deleted_at'    => 'datetime',
    ];

    /**
     * Sortable columns.
     *
     * @var array
     */
    public $sortable = ['name', 'from_date', 'to_date', 'payment_date', 'employees.name', 'status', 'amount'];

    /**
     * Clonable relationships.
     *
     * @var array
     */
    public $cloneable_relations = ['benefits', 'deductions', 'employees'];

    public function employees()
    {
        return $this->hasMany('Modules\Payroll\Models\RunPayroll\RunPayrollEmployee', 'run_payroll_id', 'id');
    }

    public function benefits()
    {
        return $this->hasMany('Modules\Payroll\Models\RunPayroll\RunPayrollEmployeeBenefit', 'run_payroll_id', 'id');
    }

    public function deductions()
    {
        return $this->hasMany('Modules\Payroll\Models\RunPayroll\RunPayrollEmployeeDeduction', 'run_payroll_id', 'id');
    }

    public function category(): BelongsTo
    {
        return $this->belongsTo('App\Models\Setting\Category');
    }

    public function account(): BelongsTo
    {
        return $this->belongsTo('App\Models\Banking\Account');
    }

    public function currency(): BelongsTo
    {
        return $this->belongsTo('App\Models\Setting\Currency', 'currency_code', 'code');
    }

    public function pay_calendar(): BelongsTo
    {
        return $this->belongsTo('Modules\Payroll\Models\PayCalendar\PayCalendar');
    }

    public function payment(): BelongsTo
    {
        return $this->belongsTo(Transaction::class);
    }

    public function getStatusAttribute($value): string
    {
        $status = $value;

        if (empty($status)) {
            $status = 'not_approved';
        }

        return $status;
    }

    public function getStatusLabelAttribute(): string
    {
        switch ($this->status) {
            case 'approved':
                $status_label = 'status-success';
                break;
            case 'not_approved':
            default:
                $status_label = 'status-danger';
                break;
        }

        return $status_label;
    }

    public function getAttachmentAttribute($value = null)
    {
        if (!empty($value) && !$this->hasMedia('attachment')) {
            return $value;
        } elseif (!$this->hasMedia('attachment')) {
            return false;
        }

        return $this->getMedia('attachment')->all();
    }

    public function delete_attachment()
    {
        if ($attachments = $this->attachment) {
            foreach ($attachments as $file) {
                MediaModel::where('id', $file->id)->delete();
            }
        }
    }

    /**
     * Get the line actions.
     *
     * @return array
     */
    public function getLineActionsAttribute()
    {
        $actions = [];

        if (($this->status != 'not_approved')) {
            $actions[] = [
                'title' => trans('payroll::run-payrolls.not_approved'),
                'icon' => 'groups',
                'url' => route('payroll.run-payrolls.not.approved', $this->id),
                'permission' => 'update-payroll-run-payrolls',
            ];
        };

        $actions[] = [
            'title' => trans('general.edit'),
            'icon' => 'edit',
            'url' => route('payroll.run-payrolls.edit', $this->id),
            'permission' => 'update-payroll-run-payrolls',
        ];

        $actions[] = [
            'title' => trans('general.duplicate'),
            'icon' => 'file_copy',
            'url' => route('payroll.run-payrolls.duplicate', $this->id),
            'permission' => 'create-payroll-run-payrolls',
        ];

        $actions[] = [
            'type' => 'delete',
            'title' => trans_choice('payroll::general.run_payrolls', 1),
            'icon' => 'delete',
            'route' => 'payroll.run-payrolls.destroy',
            'permission' => 'delete-payroll-run-payrolls',
            'model' => $this,
        ];

        return $actions;
    }

    public static function newFactory(): Factory
    {
        return \Modules\Payroll\Database\Factories\RunPayroll::new();
    }

    public function onCloning($src, $child = null)
    {
        $this->payment_id = null;
        $this->status = self::STATUS_NOT_APPROVED;
        $this->name = $this->getNextRunPayrollNumber();
    }
}
