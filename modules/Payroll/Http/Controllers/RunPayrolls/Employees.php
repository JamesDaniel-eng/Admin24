...
use Modules\Payroll\Jobs\RunPayroll\UpdateRunPayroll;
...
class Employees extends Controller
{
    ...
    public function create(PayCalendar $payCalendar): JsonResponse
    {
        ...
        $pay_calendar_employees = PayCalendarEmployee::where('pay_calendar_id', $payCalendar->id)
            ->with(['employee.department', 'employee.contact'])
            ->get();
        ...
        foreach ($pay_calendar_employees as $pay_calendar_employee) {
            ...
            $benefits = $this->getBenefits($pay_calendar->type, $employee);

            $deductions = $this->getDeductions($pay_calendar->type, $employee);

            $employees_data[] = [
                'employee'   => $employee,
                'benefits'   => $benefits,
                'deductions' => $deductions,
                'totals'     => $this->getSalary($pay_calendar->type, $employee) + $benefits - $deductions,
            ];
        }

        $number = $this->getNextRunPayrollNumber();

        $html = view('payroll::modals.run-payrolls.employees.create', compact('pay_calendar', 'employees_data', 'number'))->render();
    ...
    public function edit(RunPayroll $runPayroll): JsonResponse
    {
        ...
        $html = view('payroll::modals.run-payrolls.employees.edit', compact('run_payroll', 'employees'))->render();
    ...
    {
        $response = $this->ajaxDispatch(new UpdateRunPayroll($runPayroll, $request));

        if ($response['success']) {
            $response['redirect'] = route('payroll.run-payrolls.variables.edit', $runPayroll->id);

            $message = trans('messages.success.updated', ['type' => trans_choice('payroll::general.pay_calendars', 1)]);

            flash($message)->success();
        } else {
            $response['redirect'] = route('payroll.run-payrolls.employees.edit', $runPayroll->id);
    ...
    public function employee(RunPayroll $runPayroll, Employee $employee): JsonResponse
    {
        ...
        $salary_amount = $this->getSalary($runPayroll->pay_calendar->type, $employee);

        $salary = money($salary_amount, $currency_code, true)->format();
