...
class Approvals extends Controller
{
    ...
    public function update(PayCalendar $payCalendar, RunPayroll $runPayroll, Request $request): JsonResponse
    {
        ...
        foreach ($runPayroll->employees as $employee) {
            $employee_currency = $employee->employee->contact->currency;

            if ($runPayroll->currency_code != $employee_currency->code) {
                $employee->total = ($employee->total / $employee_currency->rate) * $runPayroll->currency->rate;
            };
        ...
        $data = [
            ...
            'created_by'     => user()->id,
            'created_from'   => source_name('payroll'),
            'reference'      => trans('payroll::run-payrolls.reference', ['id' => $runPayroll->name]),
        ];

        $transaction = Transaction::find($runPayroll->payment_id);

        if ($transaction) {
            $payment = $this->dispatch(new UpdateTransaction($transaction, $data));
        } else {
            $payment = $this->dispatch(new CreateTransaction($data));
        }
