    ...
    <x-table.tbody>
        <x-table.tr class="relative flex items-center px-1 group/actions border-b" v-for="(row, index) in form.items" ::index="index">
            <x-table.td class="w-10/12">
                <x-form.group.text name="items[][pay_item]" value="" data-item="pay_item" v-model="row.pay_item" />
            </x-table.td>

            <x-table.td class="w-2/12 none-truncate" override="class">
                <x-button type="button" @click="onDeleteBenefit(index)" class="px-3 py-1.5 mb-3 sm:mt-2 sm:mb-0 rounded-xl text-sm font-medium leading-6 hover:bg-gray-200 disabled:bg-gray-50" override="class">
        ...
        <x-table.tr id="addItem">
            <x-table.td class="w-full">
                <x-button type="button" override="class" @click="onAddBenefit" class="w-full text-secondary flex items-center justify-center" title="{{ trans('general.add') }}">
