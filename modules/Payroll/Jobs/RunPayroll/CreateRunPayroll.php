...
class CreateRunPayroll extends Job
{
    ...
    {
        $account = Account::find($this->request['account_id'] ?? 0);

        $this->request['currency_code'] = $account->currency->code ?? setting('default.currency');
        $this->request['currency_rate'] = $account->currency->rate ?? 1;
