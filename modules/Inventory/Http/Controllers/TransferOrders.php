...
use Modules\Inventory\Imports\TransferOrders\TransferOrders as Import;
use Modules\Inventory\Exports\TransferOrders\TransferOrders as Export;
...
class TransferOrders extends Controller
{
    ...
    {
        $transfer_order = TransferOrder::with('transfer_order_items.item')->findOrFail($id);
    ...
    {
        $response = $this->importExcel(new Import, $request, trans_choice('inventory::general.transfer_orders', 2));
        ...
        if ($response['success']) {
            ...
            $response['redirect'] = route('import.create', ['inventory', 'transfer-orders']);
    ...
    {
        return $this->exportExcel(new Export, trans_choice('inventory::general.transfer_orders', 2));
    }

    public function markRead(IlluminateRequest $request)
    {
