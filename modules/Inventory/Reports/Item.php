    ...
    public function setData()
    {
        ...
        foreach ($items as $key => $item) {
            ...
            foreach ($histories as $history) {
                ...
                if (
                    $history->type_type == 'App\Models\Common\Item' || 
                    $history->type_type == 'Modules\Inventory\Models\Item'
                ) {
                    ...
                    if ($document_item->type == Document::BILL_TYPE) {
                        $this->row_values['item'][$item->id][$date] += $history->quantity;
                        $this->footer_totals['item'][$date] += $history->quantity;
                    } elseif ($document_item->type == Document::INVOICE_TYPE || $document_item->type == 'pos-order') {
                        $this->row_values['item'][$item->id][$date] -= $history->quantity;
                        $this->footer_totals['item'][$date] -= $history->quantity;
                    }
