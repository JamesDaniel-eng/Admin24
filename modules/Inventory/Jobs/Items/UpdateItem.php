...
class UpdateItem extends Job implements ShouldUpdate
{
    ...
    public function handle(): Item
    {
        ...
        \DB::transaction(function () {
            ...
            // Update associated double-entry accounts if provided
            if ($this->request->has('de_income_account_id') && $this->request->de_income_account_id) {
                AccountItem::updateOrCreate(
                    ['company_id' => company_id(), 'item_id' => $this->model->id, 'type' => 'income'],
                    ['account_id' => $this->request->de_income_account_id]
                );
            }

            if ($this->request->has('de_expense_account_id') && $this->request->de_expense_account_id) {
                AccountItem::updateOrCreate(
                    ['company_id' => company_id(), 'item_id' => $this->model->id, 'type' => 'expense'],
                    ['account_id' => $this->request->de_expense_account_id]
                );
            }
                ...
                foreach ($this->model->inventory()->pluck('id')->toArray() as $inventory_item_id) {
                    if (in_array($inventory_item_id, $request_item_id)) {
                        foreach ($this->request->items as $request_item) {
                            ...
                            if (in_array($request_item['warehouse_id'], $this->model->inventory()->pluck('warehouse_id')->toArray()) || isset($request_item['id'])) {
