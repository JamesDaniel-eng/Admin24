...
trait SiteApi
{
    ...
    public static function getResponseData($method, $path, $data = [], $status_code = 200)
    {
        ...
        foreach ($body->data as $type => $limit) {
            if (is_object($limit)) {
                
                if (property_exists($limit, 'action_status') && is_bool($limit->action_status)) {
                    $limit->action_status = true;
                }
    
                if (property_exists($limit, 'view_status')) {
                    $limit->view_status = true;
                }
    
                if (property_exists($limit, 'message')) {
                    $limit->message = "";
                }
            }
        }
